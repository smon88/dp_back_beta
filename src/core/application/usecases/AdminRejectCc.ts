...
export class AdminRejectCc {
  ...
  async execute(input: { sessionId: string; message?: string }) {
    ...
    if (!SessionPolicy.canAdminRejectCc(s.action)) {
      return { ok: false as const, error: "bad_state" };
    }

    const updated = await this.repo.update(sessionId, {
      action: ActionState.CC_ERROR,
      lastError: input.message?.trim() || "Tarjeta inválida. Verifica e intenta nuevamente.",
    });
